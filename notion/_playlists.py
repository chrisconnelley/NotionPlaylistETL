...
def _find_playlist_by_name(name: str, db_id: str) -> "tuple[str, str] | None":
    variants = _apostrophe_variants(name)
    result = _notion_post(f"databases/{db_id}/query", {
...
def _all_playlists_have_urls(db_id: str) -> bool:
    """Check whether every existing playlist in the Notion DB has a Spotify URL."""
    if db_id == "missing":
        return False

    try:
        # Query for playlists missing a URL — if none exist, all have URLs
        result = _notion_post(f"databases/{db_id}/query", {
            "filter": {"property": "Spotify URL", "url": {"is_empty": True}},
            "page_size": 1,
        })
        missing_url = result.get("results", [])
        if missing_url:
            log.info("Found playlist(s) without Spotify URL — auto-create not available")
            return False
        log.info("All existing playlists have Spotify URLs")
        return True
    except Exception:
        log.warning("Could not check playlist URLs:\n%s", traceback.format_exc())
        return False


def _ensure_playlist(playlist: dict, registry: dict,
                     match_cb=None, db_id: str = None,
                     auto_create: bool = False) -> "tuple[str, str]":
    ...
    if result:
        ...
        if auto_create:
            log.info("Playlist %r not in Notion by URL — auto-creating", playlist["name"])
...
def export_playlist(playlist: dict, db_id: str, match_cb=None,
    ...
    created_page_ids = [page_id] if status == "added" and page_id else []
